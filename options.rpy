...
init -1 python hide:
    ...
    config.window_title = u"cruel reality"

    # Эти управляют именем и версией игры, которые указываются
    # в журналах отладки.
    config.name = "cruel reality"
    config.version = "1.0"
    ...
    style.mm_menu_frame.xpos = 1
    style.mm_menu_frame.xanchor = 1
    style.mm_menu_frame.ypos = 2
    style.mm_menu_frame.yanchor = 1
    ...
    config.main_menu_music = "kis.ogg"
...
python early:
    config.save_directory = "mvn1-1458541438"
...
init python:
    ...
    build.directory_name = "mvn1-1.0"

    ## The name that's uses for executables - the program that users will run
    ## to start the game. For example, if this is 'mygame', then on Windows,
    ## users can click 'mygame.exe' to start the game.
    build.executable_name = "mvn1"
